...
import { getFirestore, Firestore, doc, collection, setDoc, addDoc, arrayUnion } from 'firebase/firestore';
...
export const appId = (typeof __app_id !== 'undefined' ? __app_id : getEnv('VITE_APP_ID')) || DEFAULT_APP_ID;
const rawConfig = (typeof __firebase_config !== 'undefined' ? __firebase_config : getEnv('VITE_FIREBASE_CONFIG')) || DEFAULT_CONFIG;
...
export const initFirebase = async (): Promise<{ db: Firestore | null, auth: Auth | null, userId: string }> => {
  // --- OFFLINE MODE FALLBACK ---
  if (Object.keys(firebaseConfig).length === 0) {
    console.warn("⚠️ Firebase config missing. Initializing in OFFLINE GUEST MODE.");
    return { 
      db: null, 
      auth: null, 
...
export const ensureFamilyExists = async (db: Firestore | null, familyId: string, userId: string) => {
