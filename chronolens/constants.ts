...
export const MOCK_PHOTOS: PhotoItem[] = Array.from({ length: 30 }).map(
  (_, i) => {
    const year = 2024 - i * 5;
    ...
    const lat = isOld
      ? 48.8566 + Math.random() * 5
      : 40.7128 + Math.random() * 5; // Parisish vs NYCish
    const lng = isOld
      ? 2.3522 + Math.random() * 5
      : -74.006 + Math.random() * 5;
